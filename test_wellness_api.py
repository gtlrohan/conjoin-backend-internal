...
def test_wellness_api():
    ...
    login_response = requests.post(
        f"{BASE_URL}/auth/login", headers={"Content-Type": "application/json"}, json={"email": TEST_EMAIL, "password": TEST_PASSWORD}
    )
    ...
    wellness_data = {"energy_level": 7.5, "stress_level": 4.2}

    create_response = requests.post(f"{BASE_URL}/wellness/daily-metrics", headers={**headers, "Content-Type": "application/json"}, json=wellness_data)
    ...
    latest_response = requests.get(f"{BASE_URL}/wellness/daily-metrics/latest", headers=headers)
    ...
    history_response = requests.get(f"{BASE_URL}/wellness/daily-metrics?limit=10", headers=headers)
    ...
    stats_response = requests.get(f"{BASE_URL}/wellness/stats?days=30", headers=headers)
    ...
    updated_wellness_data = {"energy_level": 8.3, "stress_level": 3.7}

    update_response = requests.post(
        f"{BASE_URL}/wellness/daily-metrics", headers={**headers, "Content-Type": "application/json"}, json=updated_wellness_data
    )
