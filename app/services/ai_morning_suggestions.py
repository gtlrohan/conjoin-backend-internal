...
from app.postgres.crud.card import create_card_details, retrieve_all_card_details
from app.postgres.models.card import SpecialActions
from app.postgres.schema.card import TimeOfDay
...
def get_kevin_ai_suggestions(
    db: Session, user_id: int, cognitive_fingerprint: Dict[str, float], daily_state: Dict[str, float]
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are Kevin's AI mentor. Respond ONLY with valid JSON as specified in the prompt."},
        ...
        try:
            ...
            card_suggestions = convert_ai_to_card_format(db, user_id, suggestions)
...
def convert_ai_to_card_format(db: Session, user_id: int, ai_suggestions: Dict) -> List[Dict]:
    """
    ...
    try:
        ...
        for i, (_key, suggestion) in enumerate(ai_suggestions.items()):
            ...
            try:
                new_card = create_card_details(
                    ...
                    details={},
                    description="AI-generated suggestion based on your current state",
            ...
            if matched_card:
                ...
                if i < len(time_slots):
                    ...
                    suggestion_time = base_datetime.replace(hour=hour, minute=minute, second=0, microsecond=0)
                else:
                    # Fallback for extra suggestions
                    suggestion_time = base_datetime.replace(hour=9 + (i * 2), minute=0, second=0, microsecond=0)
