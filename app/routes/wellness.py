...
def create_or_update_daily_wellness(
    ...
    try:
        ...
        wellness_metrics = create_daily_wellness_metrics(
            db=db, 
            user_id=user_id, 
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to create wellness metrics: {str(e)}"
        )
...
def get_wellness_history(
    ...
    try:
        ...
        metrics = get_user_wellness_metrics(
            db=db, 
            user_id=user_id, 
            limit=limit, 
            offset=offset
        ...
        return DailyWellnessListResponse(
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to retrieve wellness metrics: {str(e)}"
        )
...
def get_wellness_by_date_range(
    ...
    try:
        ...
        metrics = get_wellness_metrics_by_date_range(
            db=db, 
            user_id=user_id, 
...
def get_latest_wellness(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to retrieve latest wellness metrics: {str(e)}"
        )
...
def get_wellness_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to retrieve wellness statistics: {str(e)}"
        )
...
def delete_wellness_entry(
    ...
    try:
        ...
        success = delete_wellness_metrics(
            db=db, 
            user_id=user_id, 
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, 
                detail="Wellness metrics entry not found"
            )
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to delete wellness metrics: {str(e)}"
        )
