...
from app.postgres.crud.gpt import get_recent_voice_sessions
...
def _detect_plan_today_query(text: str) -> bool:
    if not text:
        return False
    t = text.lower().strip()
    return ("plan" in t and ("today" in t or "for today" in t)) or t in {"what is my plan today", "whats my plan today", "my plan today"}
...
async def prompt_chatGPT(body: GPTRequest, access_token: str = Depends(JWTBearer()), db: Session = Depends(get_db)):
    """
...
async def gpt_kevin_scenario(body: GPTRequest, access_token: str = Depends(JWTBearer()), db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        if not markdown_file_path.exists():
            raise HTTPException(status_code=404, detail="Kevin's daily scenarios markdown file not found")
