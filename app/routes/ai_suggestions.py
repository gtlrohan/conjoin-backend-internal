    ...
    **Returns:**
    ...
    try:
        ...
        if not cognitive_fingerprint_record:
            raise HTTPException(status_code=404, detail="Cognitive fingerprint not found. Please complete your profile first.")

        # Get user's latest wellness metrics
        latest_wellness = get_latest_wellness_metrics(db, user_id)
        if not latest_wellness:
            raise HTTPException(status_code=404, detail="Wellness data not found. Please log your daily energy and stress levels first.")
        ...
        daily_state = {"energy_level": float(latest_wellness.energy_level), "stress_level": float(latest_wellness.stress_level)}

        # Get AI suggestions
        suggestions, error = get_kevin_ai_suggestions(db, user_id, cognitive_fingerprint, daily_state)

        if error:
            raise HTTPException(status_code=500, detail=f"Failed to generate AI suggestions: {error}")
        ...
        if not suggestions or len(suggestions) < 2:
            raise HTTPException(status_code=500, detail="Failed to generate enough AI suggestions")
    ...
    **Returns:**
    ...
    try:
        ...
        if not suggestions or len(suggestions) < 5:
            raise HTTPException(status_code=500, detail="Failed to generate enough AI suggestions for alternatives")
...
def get_user_current_state(
    ...
    try:
        ...
        return {
            ...
            if cognitive_fingerprint_record
            else None,
            "latest_wellness": {
