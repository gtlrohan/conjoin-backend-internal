...
async def test_mentor_messages():
    ...
    """
    return {"message": "Mentor messages router is working!", "status": "success", "timestamp": datetime.now().isoformat()}


@router.get("/", response_model=MentorMessagesResponse)
async def get_user_mentor_messages(limit: int = 50, offset: int = 0, db: Session = Depends(get_db), access_token: str = Depends(JWTBearer())):
    """
    ...
    try:
        ...
        return MentorMessagesResponse(messages=messages, total_count=total_count, status="success")
...
async def save_voice_session_to_chat(body: VoiceTranscriptRequest, db: Session = Depends(get_db), access_token: str = Depends(JWTBearer())):
    """
    ...
    try:
        ...
        mentor_message = create_voice_session_summary(db=db, user_id=user_id, session_id=body.session_id, conversation=body.conversation)

        logger.info(f"Voice session {body.session_id} saved as mentor message for user {user_id}")

        return {"status": "success", "message": "Voice session saved to mentor chat", "message_id": mentor_message.id}
