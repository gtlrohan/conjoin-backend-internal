...
from app.postgres.models.gpt import MentorMessage as MentorMessageModel
...
def get_mentor_messages(
    ...
    return db.query(MentorMessage).filter(
        MentorMessage.user_id == user_id
    ).order_by(desc(MentorMessage.timestamp)).offset(offset).limit(limit).all()
...
def get_recent_voice_sessions(
    ...
    return db.query(MentorMessage).filter(
        MentorMessage.user_id == user_id,
        MentorMessage.message_type == 'voice_session',
        MentorMessage.timestamp >= cutoff_date
    ).order_by(desc(MentorMessage.timestamp)).limit(limit).all()


def create_voice_session_summary(
    db: Session,
    user_id: int,
    session_id: str,
...
def delete_mentor_message(db: Session, message_id: int, user_id: int) -> bool:
    ...
    db_message = db.query(MentorMessage).filter(
