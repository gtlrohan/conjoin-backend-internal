...
def create_daily_wellness_metrics(db: Session, user_id: int, wellness_data: DailyWellnessRequest) -> DailyWellnessMetrics:
    """
    ...
    existing_entry = db.query(DailyWellnessMetrics).filter(DailyWellnessMetrics.user_id == user_id, DailyWellnessMetrics.date == today).first()
    ...
    if existing_entry:
        ...
        db_wellness = DailyWellnessMetrics(
            user_id=user_id, energy_level=wellness_data.energy_level, stress_level=wellness_data.stress_level, date=today
        )
        db.add(db_wellness)
        db.commit()
...
def get_user_wellness_metrics(db: Session, user_id: int, limit: int = 30, offset: int = 0) -> List[DailyWellnessMetrics]:
    """
    Retrieve wellness metrics for a user, ordered by date (most recent first).
    """
    return (
        db.query(DailyWellnessMetrics)
        .filter(DailyWellnessMetrics.user_id == user_id)
        .order_by(desc(DailyWellnessMetrics.date))
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def get_wellness_metrics_by_date_range(db: Session, user_id: int, start_date: date, end_date: date) -> List[DailyWellnessMetrics]:
    """
    Retrieve wellness metrics for a user within a specific date range.
    """
    return (
        db.query(DailyWellnessMetrics)
        .filter(DailyWellnessMetrics.user_id == user_id, DailyWellnessMetrics.date >= start_date, DailyWellnessMetrics.date <= end_date)
        .order_by(desc(DailyWellnessMetrics.date))
        .all()
    )
...
def get_latest_wellness_metrics(db: Session, user_id: int) -> Optional[DailyWellnessMetrics]:
    ...
    """
    return db.query(DailyWellnessMetrics).filter(DailyWellnessMetrics.user_id == user_id).order_by(desc(DailyWellnessMetrics.date)).first()
...
def get_wellness_stats(db: Session, user_id: int, days: int = 30) -> dict:
    ...
    metrics = db.query(DailyWellnessMetrics).filter(DailyWellnessMetrics.user_id == user_id, DailyWellnessMetrics.date >= start_date).all()
...
def delete_wellness_metrics(db: Session, user_id: int, metrics_id: int) -> bool:
    ...
    """
    wellness_entry = db.query(DailyWellnessMetrics).filter(DailyWellnessMetrics.id == metrics_id, DailyWellnessMetrics.user_id == user_id).first()
