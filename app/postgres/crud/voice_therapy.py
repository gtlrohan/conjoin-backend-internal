...
from app.postgres.models.voice_therapy import VoiceSessionEndRequest, VoiceSessionMessage
...
def get_voice_therapy_session(db: Session, session_id: str) -> Optional[VoiceTherapySession]:
    ...
    return db.query(VoiceTherapySession).filter(
...
def get_user_voice_therapy_sessions(
    db: Session,
    user_id: int,
    limit: int = 20,
    offset: int = 0
) -> List[VoiceTherapySession]:
    """Get voice therapy sessions for a user."""
    return db.query(VoiceTherapySession).filter(
        VoiceTherapySession.user_id == user_id
    ).order_by(desc(VoiceTherapySession.start_time)).offset(offset).limit(limit).all()
...
def process_voice_transcript_for_chat(
    db: Session, 
    user_id: int, 
    session_id: str,
    transcript: List[VoiceSessionMessage]
) -> str:
